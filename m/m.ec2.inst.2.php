...
<div data-role="page" id="ec2" data-theme="a">
  ...
  <div data-role="content" data-theme="a">
    <div class="content-primary">
      <ul data-role="listview" data-theme="a" data-divider-theme="a" data-inset="true" id="instances">
        <?php
          $res = $ec2->describe_instances();
            foreach ($res->body->reservationSet->item as $item) {
              ...
              $dict->Instance = "";
              $dict->State = "";
              if (isset($iitem->tagSet->item->value)) {
                $dict->Name = fempty($iitem->tagSet->item->value);
              }
              if (isset($iitem->instanceId)) {
                $dict->Instance = fempty($iitem->instanceId);
              }
              if (isset($iitem->instanceState->name)) {
                $dict->State = fempty($iitem->instanceState->name);
              }
        ?>
          <?php
          ...
          ?>
          <li data-icon="<?php echo($statusIcon); ?>"><a href="m.ec2.inst.view.php?id=<?php echo($dict->Instance); ?>" data-transition="slide"><?php recho($dict->Name, $dict->Instance); ?></a></li>
        <?php
            }
        ?>
      </ul>
